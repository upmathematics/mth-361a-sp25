...
```{r cs-line-model, fig.align='center', fig.height=8, fig.width=8, message=FALSE, warning=FALSE, out.width="40%"}
par(mar=c(4,4,1,1), las=1, mgp=c(2.5,0.7,0), cex.lab = 1.5, cex.axis = 1.5)
plot(coast_starlight$travel_time~ coast_starlight$dist, xlab = "Distance (miles)", ylab = "Travel Time (minutes)", pch=19, col=COL[1,2], cex=2)
lm_cs = lm(coast_starlight$travel_time~ coast_starlight$dist)
abline(lm_cs, col = COL[4], lwd = 3)
```
