...
```{r "fig-histogram-x", out.width = "70%", fig.align = "center", fig.path="1-files/", dev=c("png")}
library(MASS)
...
* The sum of the probabilities for all outcomes in the sample space is equal to 1: $$P(X) = 1.$$
...
What is the probability for our fair coin? We need to determine the **probability** for each **outcome**.
  
* For a fair coin, $P(H) = \frac{1}{2}$ and $P(T) = \frac{1}{2}$ which does not change each flip. We call this **independent** events.
